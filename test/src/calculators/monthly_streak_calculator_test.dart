  ...
  group('MonthlyStreakCalculator', () {
    group('Validation', () {
      ...
      test('invalid streak target throws ArgumentError', () {
        ...
        expect(
            () => calculator.calculateStreak(dates, 29), throwsArgumentError);
        expect(
            () => calculator.calculateStreak(dates, -1), throwsArgumentError);
      });
    ...
    group('Consecutive months', () {
      ...
      test('streak broken by insufficient month', () {
        final dates = datesInMonth(2025, 7, [1, 5, 10, 15]) // July: 4 days
            .union(
                datesInMonth(2025, 8, [2, 8])) // August: 2 days (breaks streak)
      ...
      test('gap in months breaks current streak', () {
        final dates = datesInMonth(2025, 6, [1, 5, 10, 15]) // June: 4 days
            .union(datesInMonth(2025, 9,
                [3, 9, 15, 21])); // September: 4 days (gap in July-Aug)
    ...
    group('Best streak calculation', () {
      ...
      test('multiple streaks - longest wins', () {
        final dates = datesInMonth(2025, 1, [1, 5, 10, 15]) // Jan: 4 days
            .union(datesInMonth(
                2025, 2, [2, 8, 14, 20])) // Feb: 4 days (streak of 2)
            ...
            .union(datesInMonth(
                2025, 7, [5, 12, 19, 26])); // Jul: 4 days (streak of 4)
    ...
    group('Month variations (28-31 days)', () {
      test('february leap year vs non-leap year', () {
        // Leap year 2024
        final leapDates =
            datesInMonth(2024, 2, [1, 5, 10, 15, 20, 25, 29]); // 7 days
        final leapResult = calculator.calculateStreak(leapDates, 5);

        // Non-leap year 2025
        final nonLeapDates =
            datesInMonth(2025, 2, [1, 5, 10, 15, 20, 25]); // 6 days
      ...
      test('february with exactly 28 days target', () {
        final dates =
            List.generate(28, (i) => DateTime(2025, 2, i + 1)).toSet();
      ...
      test('different month lengths handle targets correctly', () {
        final dates = datesInMonth(
                2025, 1, List.generate(31, (i) => i + 1)) // Jan: 31 days
            .union(datesInMonth(
                2025, 2, List.generate(28, (i) => i + 1))) // Feb: 28 days
            .union(datesInMonth(
                2025, 3, List.generate(31, (i) => i + 1))); // Mar: 31 days
    ...
    group('Year boundary scenarios', () {
      test('streak spanning years', () {
        final dates = datesInMonth(
                2024, 11, [1, 8, 15, 22, 29]) // Nov 2024: 5 days
            .union(
                datesInMonth(2024, 12, [3, 10, 17, 24, 31])) // Dec 2024: 5 days
            .union(
                datesInMonth(2025, 1, [2, 9, 16, 23, 30])); // Jan 2025: 5 days
    ...
    group('Edge cases', () {
      ...
      test('long streak maintains accuracy', () {
        ...
        for (int month = 1; month <= 12; month++) {
          dates.addAll(
              datesInMonth(2024, month, [1, 8, 15, 22])); // 4 days each month
        }
    ...
    group('Current streak logic', () {
      test('current month gap breaks current streak', () {
        final dates = datesInMonth(
            2025, 7, [1, 8, 15, 22]); // July: 4 days, gap to current
