...
class MonthlyStreakCalculator {
  ...
  StreakResult calculateStreak(
      Set<DateTime> normalizedDates, int streakTarget) {
  ...
  Map<int, int> _groupDatesByMonth(Set<DateTime> dates) {
    ...
    for (final date in dates) {
      final monthKey =
          date.year * 100 + date.month; // Inline month key calculation
      monthlyActiveDays.update(monthKey, (count) => count + 1,
          ifAbsent: () => 1);
    }
  ...
  int _calculateBestStreak(
    ...
    final allMonthsInRange =
        _generateCompleteMonthRange(sortedMonths.first, sortedMonths.last);
